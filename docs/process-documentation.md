...
| Weekend rule | Skip Saturday/Sunday. Consolidate Friday-night → Sunday-night messages into the **Monday 11:00 AM** post. |
...
1. <Category>: <one-line actionable summary>. [Status: Done/Fixed.] Priority: <High|Medium|Low>
Channel: <Teams channel name>
2. <Category>: <…>. Priority: <…>

<NextInternalName> (<Role>):
Channel: <…>
1. <Category>: <…>. Priority: <…>
```

Rules:
- Group by **internal owner** (Tamzida, Ashik, Rajib, Rezvi, Pranav), in that order.
- **Akramol** (Manager) appears in the payload only when explicitly assigned in a thread.
- Within an owner, restate `Channel:` before each item (items may come from different channels).
- One line per task: `<n>. <Category>: <summary>. Priority: <level>`.
...
- Keep it plain text so it pastes cleanly into Slack (no heavy Markdown; Slack renders `-`/`*`
  bullets and `**` differently).
...
|---|---|
| Explicit @mention / "X vai please…" | the named internal person |
...
| Test/verify/regression/"how long to QA" | Tamzida (QA) |
| Timeline to client, quote, approvals, cross-team coordination, get sign-off | Pranav (PM) |
| Data fix / setting change on live (delete SMS, update urgent phone #) | Tamzida (QA, has DB access) |
| Unassigned in thread | infer from subsystem table above; if still unclear, **Pranav** (PM) |
...
A single client ask can generate **parallel items for multiple owners** — e.g. a dev task to build
it *and* a PM task to get client approval or send a quote.
